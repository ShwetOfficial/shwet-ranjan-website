...
> **Technology Stack**: Next.js 14 (App Router), TypeScript, Tailwind CSS, Framer Motion, HTML5 Canvas (3D Math), Lucide Icons.
...
The primary single-page experience containing 9 high-impact interactive sections:
...
2. **3D Hero Section**: Features an edge-to-edge (`100vw`) 3D Perspective Cyber Grid & Horizon Floor Canvas (`components/hero-3d-background.tsx`) that reacts dynamically to mouse movement and page scroll at 60fps.
3. **Interactive Product Showcase (`#projects`)**: Live interactive simulators for all 5 built platforms with 3D tilt perspective cards, category filters, and modal architecture specs.
...
8. **Public Equity Case Studies (`#stock-case-studies`)**: Warren Buffett-style fundamental valuation deep dives on **ITC Limited** (₹272 entry, ₹368 DCF intrinsic value, 26% safety margin) and **HDFC Bank** (₹728 entry, ₹734 residual earnings base value, ROE recovery thesis).
9. **Editorial Insights (`#insights`)**: Essays on GST reconciliation, D2C unit economics, and AI automation leverage.
10. **Direct Channel Dock & Footer (`#contact`)**: Interactive inquiry message dispatcher, live India Standard Time (IST) digital clock, social channels, and `info@shwetranjan.com` copy tool.
...
## 3. Built Production Systems & Simulators
...
## 4. Key Financial & Business Logic
...
## 5. Technical Codebase Structure

```
Shwet Ranjan Website/
├── app/
│   ├── globals.css                # Global CSS variables, Plus Jakarta Sans font, custom scrollbar
│   ├── layout.tsx                 # Root layout, Google Font imports, metadata SEO
│   ├── page.tsx                   # Main 9-section portfolio hub
│   └── investing-modeler/
...
│   ├── projects-showcase.tsx      # Projects grid with 3D tilt cards and category filters
...
└── package.json                   # Dependencies: Next 14, Framer Motion, Lucide React
```

---

## 6. How to Use This Knowledge Base with Gemini / LLMs
...
- *"How can I add a new project to `projects.ts`?"*
